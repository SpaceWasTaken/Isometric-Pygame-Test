import pygame, noise, random, copy
...
class Map:
    def __init__(self, grass_tile, water_tile, select_tile_img, tree_img, get_iso_pos, draw_order, ground_collisions, maps_surface):
        ...
        self.item_names = []
        self.del_item_names = ['Tree']
        self.add_to_next = True

    # apeend count and maxstack to list and not only if in but also if count is less than stack
                ...
                for tile in self.map[chunk]: 
                    ...
                    if tile[3] != (): 
                        ...
                        if cart_mouse_rect.colliderect(cart_tile_rect):
                            ...
                            if tile[3][0] in self.item_names:
                                for i in inventory.inv_items_:
                                    try:
                                        if inventory.inv_items_[i]['type'] == tile[3][0] and inventory.inv_items_[i]['count'] <= inventory.inv_items_[i]['max_stack']:
                                            inventory.inv_items_[i]['count'] += 1
                                    except:pass

                            else:
                                if inventory.inv_items_[slot] == {}:
                                    inventory.inv_items_[slot] = item.get_item_data(tile[3][0])
                                self.row += 1

                            for i in inventory.inv_items_:
                                try:
                                    if inventory.inv_items_[i]['type'] not in self.item_names:
                                        self.item_names.append(inventory.inv_items_[i]['type'])
                                    if inventory.inv_items_[i]['count'] >= inventory.inv_items_[i]['max_stack']:
                                        self.item_names.remove(inventory.inv_items_[i]['type'])
                                except:pass
